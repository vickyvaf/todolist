...
const register = async (req, res) => {
  ...
  try {
    ...
    return res.status(201).send(responseData(201, "SUCCESSFULLY REGISTER", null, result));
  } catch (error) {
...
const login = async (req, res) => {
  ...
  try {
    ...
    if (!checkData) {
      return res.status(401).send(responseData(401, "wrong username or password", null, null));
    }
    ...
    return res.status(200).send(responseData(200, "SUCCESSFULLY LOGIN", null, token));
  } catch (error) {
    return res.status(500).send(responseData(500, null, error.message ?? "Internal Server Error", null));
  }
}
